...
public class GenerateFiguresVictor {

	// Defining mapping dimmensions
	static String[] tecnologico = { "t-com","t-rea","t-cus","t-doc","t-mai","t-usa","t-reu","t-por","t-tri" };
	static String[] organizacional  = { "h-sup", "h-top", "h-tra", "h-att","h-cen","h-gov","h-com","h-bus","h-ven","h-tim","h-acc" };
	static String[] economico = { "e-tco", "e-mai", "e-lic", "e-sup" };

	static String[] factores = { "e-", "h-", "t-" };
	static String[] research_facets = {"r-eva", "r-phi", "r-sol", "r-opi", "r-val"};
	...
	// Input File paths
	static String fullBibPath = "C:\\Users\\malawito\\Documents\\Workspaces\\Victor\\12323759hxvmpsqvkrzz\\Bibliografia_P.bib";

	// Output File paths
	static String outputPath = "C:\\Users\\malawito\\Documents\\Workspaces\\Victor\\12323759hxvmpsqvkrzz\\figures\\";
	static List<String> allDims = new ArrayList<String>();
	...
	public static void main(String[] args) throws FileNotFoundException, IOException, TemplateException {
		...
		//allDims.addAll(Arrays.asList(tecnologico));
		//allDims.addAll(Arrays.asList(organizacional));
		//allDims.addAll(Arrays.asList(economico));
		
		allDims.addAll(Arrays.asList(factores));
		allDims.addAll(Arrays.asList(research_facets));
		
		// ------------------------------------------------------
		// ---------Where are the papers published---------------
		// ------------------------------------------------------

		//generateDetailTable(fullBibPath,allDims);
...
//		generateTrendsEvolution(fullBibPath);
	...
	public static void generateHeatMap(String DBPath, Properties abbreviation) throws IOException, TemplateException {
		Table heatmapTable = toolkit.generateHeatMapCSV(DBPath, research_facets, factores);
		heatmapTable.printCSVTable(outputPath + "heatmap.csv");

		Map<String, String> names = new HashMap<String, String>();

		names.put("r-eva", "Evaluation");
		names.put("r-phi", "Philosophical");
		names.put("r-sol", "Solution");
		names.put("r-opi", "Opinion");
		names.put("r-val", "Validation");
		names.put("e-", "Economic");
		names.put("h-", "Organizational");
		names.put("t-", "Technological ");
		
		HeatMap hm = new HeatMap("./heatmap.csv", "./heatmap.pdf", "OOSS integration factors",
				"Research types", research_facets, factores, names);
	...
	public static void generateTrendsEvolution(String DBPath) throws IOException, TemplateException {
			...
			Table heatmapTable = toolkit.generateTrendsEvolution(DBPath, allDims.toArray(new String[allDims.size()]));
...
//		resTable.printCSVTable(outputPath + "resTableConference.csv");
	...
	private static void generateDetailTable(String DBPath, List<String> SMSDimmensions) {
		Table fulltable = toolkit.generateDetailTable(DBPath, SMSDimmensions);
		fulltable.printLatexTable(outputPath + "fullDetailedTable.tex");
	}
